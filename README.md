...
- 当前机器已验证：通过 OptiScaler DXGI/Streamline spoof 成 RTX 4090 后，游戏内“光线追踪”选项可打开，是当前默认推荐 profile。
...
本工具默认使用 OptiScaler 的 DXGI/Streamline GPU spoof，把 `HTGame.exe` 看到的 GPU 名称伪装为所选目标显卡，从而解锁游戏里的光追选项。当前 WebUI 提供三档：本机原配置、`NVIDIA GeForce RTX 4090`、`NVIDIA GeForce RTX 5080 Laptop GPU`。实测结论是：RTX 4090 profile 最稳定，能正常出现“光线追踪”选择项；RTX 5080M profile 目前只能看到“全景光追”和“关”，不推荐作为默认配置。
...
4. 选择目标显卡 profile：默认推荐 RTX 4090；本机原配置用于回退识别，RTX 5080M 仅保留作实验对照。
5. 确认游戏和启动器已关闭。
6. 点击“备份并安装光追解锁”。
...
- RTX 4090：`SpoofedGPUName=NVIDIA GeForce RTX 4090`，`SpoofedDeviceId=0x2684`。当前默认推荐，实测可正常显示“光线追踪”选项。
