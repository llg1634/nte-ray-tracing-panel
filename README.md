...
`异环光追一键部署面板 / NTE Ray Tracing Panel` 是给“异环怎么开光追”“为什么没有光追选项”这类问题准备的本地 WebUI 工具。它可以一键准备 OptiScaler、安装本地 `winmm.dll`、`OptiScaler.ini`、`OptiScaler\` 和 `OptiScaler.log`，把当前已验证成功的 GPU spoof 流程做成自动写入、可选择目标显卡、可备份、可恢复的一键开启光追流程。

重要提示：光追选项通常需要默认画质预设在“极致”或以上才会出现。切到自定义低端配置后，设置界面里的选项可能消失，但已经生效的光追状态可能仍保留。
...
- 当前默认推荐：通过 OptiScaler DXGI/Streamline spoof 成 RTX 5090，写入 `SpoofedGPUName=NVIDIA GeForce RTX 5090`、`SpoofedDeviceId=0x2B85`、`DxgiVRAM=32`。
- RTX 4090 作为已验证备用 profile 保留。
...
本工具默认使用 OptiScaler 的 DXGI/Streamline GPU spoof，把 `HTGame.exe` 看到的 GPU 名称伪装为所选目标显卡，从而解锁游戏里的光追选项。当前 WebUI 提供四档：本机原配置、`NVIDIA GeForce RTX 5090`、`NVIDIA GeForce RTX 4090`、`NVIDIA GeForce RTX 5080 Laptop GPU`。默认推荐 RTX 5090；RTX 4090 作为已验证备用 profile；RTX 5080M profile 目前只能看到“全景光追”和“关”，不推荐作为默认配置。
...
- 默认只管理游戏目录内这些路径：
  ...
  - `OptiScaler.log`
  - `OptiScaler\`
- 不写入、删除或恢复 `nvngx.dll`、`dlsstweaks.ini`、`dlsstweaks.log`。
- 如果检测到 `nvngx.dll` + `dlsstweaks.ini`，视为 DLSS Panel 已安装；本工具只显示兼容状态，不接管这些文件。
- 文件归属边界：RT Panel 负责 `winmm.dll` / `OptiScaler`，DLSS Panel 负责 `nvngx.dll` / `DLSSTweaks`。
...
4. 选择目标显卡 profile：默认推荐 RTX 5090；RTX 4090 作为已验证备用，本机原配置用于回退识别，RTX 5080M 仅保留作实验对照。
5. 确认游戏和启动器已关闭。
6. 点击“备份并安装光追解锁”。
...
OptiScaler 的 GPU spoof 能改写游戏进程通过 DXGI/Streamline 读取到的显卡描述、VendorId、DeviceId 和显存信息。本工具会按 WebUI 选择生成配置。四档目标显卡：

- 本机原配置：读取当前 NVIDIA 显卡名称和 DeviceId，适合回到本机识别。
- RTX 5090：`SpoofedGPUName=NVIDIA GeForce RTX 5090`，`SpoofedDeviceId=0x2B85`，`DxgiVRAM=32`。当前默认推荐。
- RTX 4090：`SpoofedGPUName=NVIDIA GeForce RTX 4090`，`SpoofedDeviceId=0x2684`，`DxgiVRAM=16`。已验证备用 profile。
- RTX 5080M：`SpoofedGPUName=NVIDIA GeForce RTX 5080 Laptop GPU`，`SpoofedDeviceId=0x2C59`。实验性对照项，实测只出现“全景光追”和“关”，不推荐默认使用。
...
- [DLSSTweaks](https://github.com/emoose/DLSSTweaks)：相关图形注入/包装工具。`nvngx.dll` / `DLSSTweaks` 归 DLSS Panel 管理；本项目不使用 DLSSTweaks 的 DLSS 比例配置流程，也不写入、删除或恢复 `nvngx.dll`、`dlsstweaks.ini`。
